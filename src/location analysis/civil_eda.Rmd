---
title: "EDA - Civil Engineering Patents"
...
```
**Unique Patents**: 259,248  
**Unique Inventors**: 228,583  
**Unique Locations**: 38,668  
...
ggsave("figures/num_loc_CE.png", dpi = 600, width = 12, height = 10, units = "cm", scale = 1)
...
```{r}
# man with 13 locations - how many patents does he have?

df %>%
  filter(inventor_id == "fl:ja_ln:orban-4") %>%
  nrow()
...
ggsave("figures/civil_patents_per_year.png", dpi = 600) #width = 12, height = 10, units = "cm", scale = 1)
...
ggsave("figures/civil_pctF_per_year.png", dpi = 600) #width = 12, height = 10, units = "cm", scale = 1)


```

```{r}
loc_df <- read_csv("../../data/PatentsView/working/civil_inv_loc_map_data.csv")
```
```{r}
...
loc_US_all_time_F <- loc_df %>%
  group_by(location_id, city, state) %>%
  summarise(num_inv_F = sum(inv_cnt_F)) %>%
  arrange(desc(num_inv_F)) 

head(loc_US_all_time_F[ ,c(2:4)], 10)
```

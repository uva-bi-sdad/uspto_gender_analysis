...
```{r}
# upload the data
inventor_pharma <- read.csv("/project/biocomplexity/sdad/projects_data/uspto/PatentsView/working/pharma_inventors_v01.csv")
inventor_pharma$field <- "pharmaceuticals"
inventor_mechanical <- read.csv("/project/biocomplexity/sdad/projects_data/uspto/PatentsView/working/civil_inventors_v01.csv")
inventor_mechanical$field <- "civil engineering"
...
inventor_prod <- inventor %>% 
  select(inventor_id,location_id,patent_id,gender,date,field,country_inv,state) %>% 
  group_by(inventor_id,location_id,gender,date,field,country_inv,state) %>% 
  mutate(number_patent=length(unique(patent_id)), .keep = "unused") %>% 
  unique()

# country status
inventor_prod$origin <- "foreign"
inventor_prod$origin[inventor_prod$country_inv=="US"] <- "US"

# group date by period
inventor_prod <- inventor_prod %>% mutate(period=cut(date, breaks=c(1975, 1990, 2000, 2010,2021), labels=c("1976-1990","1990-2000","2000-2010","2010-2021")))
```
